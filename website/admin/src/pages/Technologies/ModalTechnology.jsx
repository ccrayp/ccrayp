...
import { getTechnologyData,  deleteTechnologyById, updateTechnologyById, newTechnologyByData } from "./utilities";
...
export default function ModalTechnology({ id = null, imgPrefix, showModal, handleCloseModal, handleOnSuccess }) {
    ...
    async function checkAuthAndLoadData() {
        ...
        try {
            const response = await getTechnologyData(id);
            setData(response || {});
        } catch (error) {
    ...
    function deleteTechnology() {
        async function deleteFunc() {
            ...
            if (res) {
                setIsLoading(true)
                try {
                    const res = await deleteTechnologyById(id)
                    ...
                    alert('Ошибка при удалении технологии:', error)
                }
                finally {
                    setIsLoading(false)
    ...
    function updateTechnology() {
        async function updateFunc() {
            ...
            if (res) {
                setIsLoading(true)
                try {
                    const res = await updateTechnologyById(id, new FormData(formRef.current))
                    ...
                    alert('Ошибка при обновлении технологии:', error)
                }
                finally {
                    setIsLoading(false)
    ...
    function newTechnology() {
        async function newFunc() {
            setIsLoading(true)
            try {
                const res = await newTechnologyByData(new FormData(formRef.current))
                if (res.status === 201) {
                    await handleOnSuccess()
                    closeModal()
                }
                ...
                alert('Ошибка при добавлении технологии:', error)
            }
            finally {
                setIsLoading(false)
            }
