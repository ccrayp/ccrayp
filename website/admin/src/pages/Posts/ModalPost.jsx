...
import { getPostData,  deletePostById, updatePostById, newPostByData } from "./utilities";
...
export default function ModalPost({ id = null, imgPrefix, showModal, handleCloseModal, handleOnSuccess }) {
    ...
    function updatePost() {
        async function updateFunc() {
            ...
            if (res) {
                setIsLoading(true)
                try {
                    const res = await updatePostById(id, new FormData(formRef.current))
                    ...
                    alert('Ошибка при обновлении записи:', error)
                }
                finally {
                    setIsLoading(false)
    ...
    function newPost() {
        async function newFunc() {
            setIsLoading(true)
            try {
                const res = await newPostByData(new FormData(formRef.current))
                if (res.status === 201) {
                    await handleOnSuccess()
                    closeModal()
                }
